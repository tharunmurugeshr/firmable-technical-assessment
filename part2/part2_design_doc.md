...
- **Cohort starting MRR grows in early months:** Customers in the same quarter don't all start
  on day 1 — some start in month 1, others in month 2 or 3. The cohort base accumulates until
  the quarter closes. NRR at month 0 reflects only the earliest starters, which is why
  `COHORT_STARTING_MRR` increases in early months before stabilizing.
